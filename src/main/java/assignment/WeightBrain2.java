...
public class WeightBrain2 implements Brain {
    ...
    public WeightBrain2(int width, int height, double[] weights) {
        ...
        if (weights == null) {
            ...
            for (int i = 0; i < numMetrics; i++) {
                this.weights[i] = 20 * Math.random() - 10;
            }
        } else {
    ...
    private double[] returnMetrics(Board newBoard, Board currentBoard, int n) {
        ...
        metrics[i++] = (newBoard.getMaxHeight() - currentBoard.getMaxHeight());       // max height
        ...
        for (int j = 1; j < newBoard.getWidth(); j++) {
            metrics[i++] = (columnHeight(newBoard, currentBoard, j));  // differences in row height
        }
